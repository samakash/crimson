...
public class MeditationSessionResource {
    ...
    public List<MeditationSession> getAllMeditationSessions(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all MeditationSessions");
        if (eagerload) {
            return meditationSessionRepository.findAllWithEagerRelationships();
        } else {
